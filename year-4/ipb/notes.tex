...
As you expand the disrtibutions, we can prove that for fixed variance, Gaussian has the highest entropy.

Densities are not probabilities. The discrete case $p(x)$ is the probability that $X = x$. For the continous case:
\begin{align*}
\int_{x_0}^{x_1} dx p(x)
\end{align*}
